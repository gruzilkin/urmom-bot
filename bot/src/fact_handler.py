...
"""
        ...
        CRITICAL: Only extract parameters if the message is an IMPERATIVE SENTENCE
        commanding the bot to store/remove facts.
        DO NOT extract parameters for ANY QUESTION.
        
        IMPORTANT: "BOT" is just a placeholder for the assistant.
        It is not a user to remember facts about.
    ...
    async def _remember_fact(self, guild_id: int, user_id: int, fact_content: str, language_name: str) -> str:
        ...
        async with self.telemetry.async_create_span("remember_fact") as span:
            ...
            if not current_memory:
                ...
                Create the memory entry maintaining third-person perspective.
                Please respond in {language_name}.
                
                For the confirmation_message field, provide a brief, friendly
                confirmation that includes the specific fact you're
                ...
                prompt = f"""
                You need to update a user's memory by incorporating new
                information.

                Current memory: {current_memory}
                New information: {fact_content}

                Merge the new information with the existing memory,
                resolving any conflicts intelligently and maintaining a
                natural narrative flow. Maintain third-person perspective.
                Please respond in {language_name}.

                For the confirmation_message field, provide a brief, friendly
                confirmation that includes the specific fact you're adding.
                For example:
                - "I'll remember that their birthday is March 15th"
                - "I'll remember that John worked at Google"
                - "I'll remember that they graduated from MIT"

                Include the actual new fact content in your confirmation
                message.
                """
    ...
    async def _forget_fact(self, guild_id: int, user_id: int, fact_content: str, language_name: str) -> str:
        ...
        async with self.telemetry.async_create_span("forget_fact") as span:
            ...
            if not current_memory:
                ...
                no_memory_prompt = f"""
                The user asked you to forget information about someone,
                but you have no memory about that user.

                Information they wanted to forget: {fact_content}

                Please respond in {language_name}. For the
                confirmation_message field, provide a brief message
                explaining that you don't have any memory about that
                user to forget. For example:
                - "I don't have any memory about that user to forget."

                Set fact_found to false and leave updated_memory empty
                since there's no memory to update.
                """
            ...
            prompt = f"""
            You need to determine if specific information exists in a
            user's memory and remove it if found.

            Current memory: {current_memory}
            Information to remove: {fact_content}

            If the information exists in the memory, remove it and
            return the updated memory with fact_found=true.
            If the information is not found, set fact_found=false
            (the updated_memory field will be ignored).
            Maintain third-person perspective.
            Please respond in {language_name}.

            For the confirmation_message field, include the specific
            fact content:
            - If fact_found=true: "I've forgotten that [specific fact]"
              (e.g., "I've forgotten that their birthday is March 15th",
              "I've forgotten that they visited Paris")
            - If fact_found=false: "I couldn't find that information in
              my memory" (e.g., "I couldn't find that information about
              their favorite food")
