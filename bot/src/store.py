    ...
    async def get_guild_config(self, guild_id: int) -> GuildConfig:
        async with self._telemetry.async_create_span("get_guild_config") as span:
            ...
            if guild_id not in self._guild_configs:
                ...
                async with self.conn.cursor() as cur:
                    await cur.execute(
                        "INSERT INTO guild_configs (guild_id) VALUES (%s) ON CONFLICT DO NOTHING", (guild_id,)
                    )
            ...
            try:
                ...
                self._telemetry.metrics.db_latency.record(
                    timer(),
                    {"operation": "get_guild_config", "guild_id": str(guild_id), "cache_outcome": cache_outcome},
                )
    ...
    async def save_guild_config(self, config: GuildConfig) -> None:
        async with self._telemetry.async_create_span("save_guild_config") as span:
            ...
            try:
                ...
                self._telemetry.metrics.db_latency.record(
                    timer(), {"operation": "save_guild_config", "guild_id": str(config.guild_id)}
                )
    ...
    async def get_user_facts(self, guild_id: int, user_id: int) -> str | None:
        ...
        async with self._telemetry.async_create_span("get_user_facts") as span:
            ...
            try:
                ...
                async with self.conn.cursor() as cur:
                    await cur.execute(
                        "SELECT memory_blob FROM user_facts WHERE guild_id = %s AND user_id = %s", (guild_id, user_id)
                    )
                    result = await cur.fetchone()
    ...
    async def save_user_facts(self, guild_id: int, user_id: int, memory_blob: str) -> None:
        ...
        async with self._telemetry.async_create_span("save_user_facts") as span:
            ...
            try:
                ...
                self._telemetry.metrics.db_latency.record(
                    timer(), {"operation": "save_user_facts", "guild_id": str(guild_id)}
                )
    ...
    async def get_chat_messages_for_date(self, guild_id: int, for_date: date) -> list[ChatMessage]:
        ...
        async with self._telemetry.async_create_span("get_chat_messages_for_date") as span:
            ...
            try:
                ...
                self._telemetry.metrics.db_latency.record(
                    timer(), {"operation": "get_chat_messages_for_date", "guild_id": str(guild_id)}
                )
    ...
    async def add_chat_message(
        ...
        async with self._telemetry.async_create_span("add_chat_message") as span:
            ...
            try:
                ...
                self._telemetry.metrics.db_latency.record(
                    timer(), {"operation": "add_chat_message", "guild_id": str(guild_id)}
                )
    ...
    async def get_daily_summaries(self, guild_id: int, for_date: date) -> dict[int, str]:
        ...
        async with self._telemetry.async_create_span("get_daily_summaries") as span:
            ...
            try:
                ...
                self._telemetry.metrics.db_latency.record(
                    timer(),
                    {"operation": "get_daily_summaries", "guild_id": str(guild_id), "cache_outcome": cache_outcome},
                )
