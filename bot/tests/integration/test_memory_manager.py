...
class Profile:
    """Describes an AI client with a name for integration subtests."""
...
class MemoryManagerTestBase(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        # parsing uses gpt-6-luna, and drives the nested DailySummaries schema through Codex.
    ...
    def _build_summary_manager(self, summary_profile: Profile) -> MemoryTestContext:
        """Build MemoryManager for daily summary integration tests."""
        store = TestStore()
        memory_manager = MemoryManager(
            telemetry=self.telemetry,
            store=store,
