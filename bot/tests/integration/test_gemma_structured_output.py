...
class TestGemmaStructuredOutput(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_structured_output_with_grounding_warning(self):
        ...
        result = await self.client.generate_content(message=message, enable_grounding=True, response_schema=YesNo)
    ...
    async def test_samples_warning(self):
        ...
        result = await self.client.generate_content(message=message, samples=samples, response_schema=YesNo)
