...
class TestGeminiModelAvailability(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_gemini_flash_model_availability(self):
        ...
        client = GeminiClient(api_key=self.api_key, model_name=model_name, temperature=0.1, telemetry=self.telemetry)

        # Simple test request - successful API call is enough
        response = await client.generate_content(message="Hello", prompt="Respond briefly.")
