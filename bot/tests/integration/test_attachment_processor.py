...
class TestAttachmentProcessorIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        if not all([gemini_api_key, gemma_model]):
            self.skipTest(
                "Missing GEMINI_API_KEY or GEMINI_GEMMA_MODEL environment variables."
            )
    ...
    async def test_process_image_attachment_from_url(self):
        ...
        # Act
        embeddings = await self.attachment_processor._process_attachments(
            [mock_attachment]
        )
    ...
    def test_process_article_embed_from_url(self):
        ...
        # Act
        embeddings = self.attachment_processor._process_embeds([mock_embed])
