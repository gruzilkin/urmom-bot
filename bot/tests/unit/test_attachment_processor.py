...
class TestAttachmentProcessorUnit(unittest.IsolatedAsyncioTestCase):
    ...
    def test_initialization(self):
        ...
        processor_5mb = AttachmentProcessor(self.mock_ai_client, self.telemetry, max_file_size_mb=5)
    ...
    def test_is_supported_image(self):
        ...
        scenarios = [
            ...
            ("Supported type, size at exact limit", "image/jpeg", 1 * 1024 * 1024, True),
            ("Supported type, size over limit", "image/gif", 1 * 1024 * 1024 + 1, False),
            ("Unsupported type, size within limit", "application/pdf", 500 * 1024, False),
        ]
        ...
        for name, content_type, size, expected in scenarios:
            with self.subTest(msg=name):
                ...
                self.assertEqual(self.attachment_processor.is_supported_image(attachment), expected)
    ...
    async def test_download_attachment_success(self, mock_get):
        ...
        # Act
        result = await self.attachment_processor.download_attachment(attachment)
    ...
    async def test_analyze_image_success(self):
        ...
        # Act
        description = await self.attachment_processor.analyze_image(attachment_data)
    ...
    async def test_analyze_image_failure(self):
        ...
        with self.assertRaises(Exception) as context:
            await self.attachment_processor.analyze_image(attachment_data)
    ...
    def test_extract_article_success(self, mock_goose):
        ...
        # Act
        content = self.attachment_processor.extract_article(url)
    ...
    def test_extract_article_caching(self, mock_goose):
        ...
        # Act
        result1 = self.attachment_processor.extract_article(url)
        result2 = self.attachment_processor.extract_article(url)
    ...
    async def test_process_all_content_mixed(self):
        ...
        with patch.object(self.attachment_processor, 'download_attachment', new_callable=AsyncMock) as mock_download:
            mock_download.return_value = AttachmentData(b'data', 'image/png', 'test.png', 4)
            
            with patch.object(self.attachment_processor, 'analyze_image', new_callable=AsyncMock) as mock_analyze:
                mock_analyze.return_value = "Image description"

                # Mock embed processing
                mock_embed = Mock()
                mock_embed.url = "http://example.com/article"
                with patch.object(self.attachment_processor, 'extract_article') as mock_extract:
