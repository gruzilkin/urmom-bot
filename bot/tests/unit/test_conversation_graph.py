...
class BaseMessageGraphTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def mock_fetch_history(self, message_id):
        ...
        older_messages = [msg for msg in self.messages.values() if msg.created_at < before_msg.created_at]
...
class TestMessageGraph(BaseMessageGraphTest):
    ...
    def test_add_node_deduplication(self):
        ...
        msg = self.create_mock_discord_message(
            msg_id=1, content="test message", author_id=1000, created_at=self.test_time
        )
    ...
    def test_reference_tracking(self):
        ...
        msg_with_ref = self.create_mock_discord_message(
            msg_id=1, content="reply message", author_id=1000, created_at=self.test_time, reference_id=123
        )

        msg_without_ref = self.create_mock_discord_message(
            msg_id=2, content="regular message", author_id=2000, created_at=self.test_time
        )
    ...
    def test_temporal_frontier_sorting(self):
        ...
        older_msg = self.create_mock_discord_message(
            msg_id=1, content="older", author_id=1000, created_at=self.test_time - datetime.timedelta(minutes=10)
        )

        newer_msg = self.create_mock_discord_message(
            msg_id=2, content="newer", author_id=2000, mentioned_user_ids=[], created_at=self.test_time
        )
    ...
    async def test_chronological_conversation(self):
        ...
        msg1 = self.create_mock_discord_message(
            msg_id=1, content="first message", author_id=1000, created_at=self.test_time - datetime.timedelta(minutes=5)
        )

        msg2 = self.create_mock_discord_message(
            msg_id=2, content="second message", author_id=2000, created_at=self.test_time
        )
...
class TestConversationGraphBuilder(BaseMessageGraphTest):
    def setUp(self):
        ...
        self.builder = ConversationGraphBuilder(
            fetch_message=self.mock_fetch_message, fetch_history=self.mock_fetch_history, telemetry=NullTelemetry()
        )
    ...
    async def test_api_efficiency_with_200_messages(self):
        ...
        counting_builder = ConversationGraphBuilder(
            fetch_message=self.mock_fetch_message, fetch_history=counting_fetch_history, telemetry=NullTelemetry()
        )
